...
class TestSizeFilter(unittest.TestCase):
    def test_size_filter(self):
        ...
        filtered_res = size_filter(seg, filtered_res, min_size=min_size, block_shape=block_shape)
        ...
        filtered_res = np.zeros_like(seg)
        filtered_res = size_filter(seg, filtered_res, max_size=max_size, block_shape=block_shape)
        _, filtered_sizes = np.unique(filtered_res, return_counts=True)
        ...
        filtered_res = np.zeros_like(seg)
        filtered_res = size_filter(seg, filtered_res, min_size=min_size, max_size=max_size, block_shape=block_shape)
        _, filtered_sizes = np.unique(filtered_res, return_counts=True)
        self.assertTrue((filtered_sizes >= min_size).all())
        filtered_sizes = filtered_sizes[1:]
        self.assertTrue((filtered_sizes <= max_size).all())
